import {
  BusinessDay,
  ChartOptions, createChart, DeepPartial, LineStyle, PriceScaleMode, Time,
} from 'lightweight-charts';

import resizeHandler from '@/components/chart/utils/resizeHandler';
...
function getValueByTime(time: Time): number {
  return areaData.find((data) => {
    const t = time as BusinessDay;
    const d = data.time as BusinessDay;
...
export default function render(
  ...
  window.addEventListener('resize', () => resizeHandler(chart));
  resizeHandler(chart);

  return new ChartRef(chart, container, tooltip, getValueByTime);
}
